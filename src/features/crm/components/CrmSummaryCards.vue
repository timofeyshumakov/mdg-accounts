<template>
  <section class="crm-summary">
    <article
      ...
      :key="metric.id"
      class="crm-summary__card"
    >
      <span
        v-if="iconById[metric.id]"
        class="crm-summary__icon-wrap"
        :style="{ backgroundColor: iconColors[index] ?? iconColors[0] }"
      >
        <img
          class="crm-summary__icon"
          :src="iconById[metric.id]"
          :alt="metric.label"
        >
      </span>
